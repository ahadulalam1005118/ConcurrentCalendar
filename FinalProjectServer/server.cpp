...
int server ::begin(std::string user_info_file_path, std::string config_file_path, std::string database_path) {
    ...
    this->events =get_events(this->database_path);
...
void server::write_events() {
    write_events_to_file();
}
void server::write_events_to_file() {
    ofstream outFile;
    const char * file = _server->database_path.c_str();
    outFile.open(file,ios::binary|ios::out);
    outFile.clear();
    if (outFile.fail()) {
        cout << "Could Not Open File\n";
    } else {
        for(int j=0; j< _server->events.size(); j++) {
            string to_write;
            string pipe = "|";
           // size_t pipe_sz = pipe.size();
            ...
            to_write+=_server->events[j].hostname+pipe+_server->events[j].date+pipe+_server->events[j].start_time+pipe+
                                _server->events[j].end_time+pipe+_server->events[j].event_desc+pipe;
            ...
            if (FD_ISSET(client[i], &rset)) {
                ...
                if (num == 0) {
                    ...
                    if(commands[0] == "r") {
                        ...
                        string event_data = commands[1];
                        vector<string> splitted_commands;
                        splitted_commands = split(commands[1], '|');
                        event new_event;
                        new_event.hostname = get_username_from_sockfd(client[i]);
                        new_event.date = splitted_commands[0];
                        new_event.start_time = splitted_commands[1];
                        new_event.end_time = splitted_commands[2];
                        new_event.event_desc = splitted_commands[3];
                        _server->events.push_back(new_event);
                        _server->write_events();
                        string to_send = "data$";
                        to_send += get_all_events_in_string(this->database_path);
                        /*send_events_to_online_users(string to_send);*/
                        for(int cnt=0; cnt<_server->online_users.size(); cnt++){
                            write(_server->online_users[cnt].sock_fd,to_send.c_str(),to_send.size());

                        }
                        //write(client[i],to_send.c_str(),to_send.size());
...
string server::get_all_events_in_string(string filename) {
...
vector<event> server::get_events(string filename) {
    vector <event> events;
    ifstream inFile;
    const char * file = filename.c_str();
    inFile.open(file,ios::binary|ios::in);
    if (inFile.fail()) {
        ...
        if (!(inFile.peek() == std::ifstream::traits_type::eof())) {
            while(getline(inFile, line)) {
                event evnt;
                vector<string>event_infos = split(line,'|');
                //user_info user = split_string_user_info(line,"|");
                if(event_infos.size() == 5) {
                    evnt.hostname = event_infos[0];
                    evnt.date = event_infos[1];
                    evnt.start_time = event_infos[2];
                    evnt.end_time = event_infos[3];
                    evnt.event_desc = event_infos[4];
