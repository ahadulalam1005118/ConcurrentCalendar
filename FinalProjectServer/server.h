...
class server {
public:
    int begin(std::string user_info_file_path, std::string config_file_path, std::string database_path);
    int run();
...
private:
    ...
    vector<config> get_configurations(std::string filename);
    vector<user_info> get_users(string filename);
    vector<event> get_events(string filename);
    string get_all_events_in_string(string filename);
    user_info get_user_info_by_name(string username);
